...
impl YieldVault {
    ...
    pub fn get_secure_price_with_metadata(env: &Env) -> Result<(i128, u32, bool), VaultError> {
        ...
        if let Some((price, timestamp)) = client.get_price(&token_addr) {
            ...
            if age_seconds < 900 {
                ...
                env.events().publish(
                    (soroban_sdk::symbol_short!("oracle"), soroban_sdk::symbol_short!("fresh")),
                    (price, confidence)
                );
            ...
            env.events().publish(
                (soroban_sdk::symbol_short!("oracle"), soroban_sdk::symbol_short!("stale")),
                age_seconds
            );
        }
        ...
        env.events().publish(
            (soroban_sdk::symbol_short!("twap"), soroban_sdk::symbol_short!("used")),
    ...
    fn update_price_history(env: &Env, price: i128, timestamp: u64, confidence: u32) {
        ...
        history.push_front(PricePoint { price, timestamp, confidence });
